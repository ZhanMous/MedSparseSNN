...
"""分析 PathMNIST 最终对比实验的稀疏性、理论 MAC 节省与单样本能耗。"""
...
def main(args):
    ...
    detailed_out = os.path.join(CSV_DIR, f'pathology_privacy_efficiency_runs_{args.training_prefix}.csv')
    with open(detailed_out, 'w', newline='') as handle:
        writer = csv.DictWriter(
            handle,
            fieldnames=[
                'dataset', 'model', 'seed', 'encoding', 'augment', 'T', 'test_acc', 'power_w',
                'latency_ms_per_sample', 'energy_mj_per_sample', 'spike_rate', 'theoretical_macs_saving',
                'ann_reference_macs', 'ann_reference_flops'
            ],
        )
        writer.writeheader()
        writer.writerows(evaluation_rows)

    summary_out = os.path.join(CSV_DIR, f'pathology_privacy_efficiency_summary_{args.training_prefix}.csv')
    with open(summary_out, 'w', newline='') as handle:
        writer = csv.writer(handle)
        writer.writerow([
            'dataset', 'model', 'repeats', 'test_acc', 'power_w', 'latency_ms_per_sample',
            'energy_mj_per_sample', 'spike_rate', 'theoretical_macs_saving'
        ])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--training-prefix', default='pathology_final_compare')
